...
import static com.katali.orderservice.testdata.TestDataFactory.*;
...
import com.katali.orderservice.domain.OrderService;
import com.katali.orderservice.domain.SecurityService;
...
class OrderControllerUnitTests {
    ...
    @BeforeEach
    void setUp() {
        given(securityService.getLoginUserName()).willReturn("Kadir");
    }

    @ParameterizedTest(name = "[{index}]-{0}")
    @MethodSource("createOrderRequestProvider")
    void shouldReturnBadRequestWhenOrderPayloadIsInvalid(CreateOrderRequest request) throws Exception {
        given(orderService.createOrder(eq("Kadir"), any(CreateOrderRequest.class)))
                .willReturn(null);
